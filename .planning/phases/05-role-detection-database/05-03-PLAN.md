...
must_haves:
  truths:
    - "ContentService existe con métodos CRUD completos para paquetes"
    - "Método create_package() crea paquete con todos los campos"
    - "Método get_package() retorna paquete por ID"
    - "Método list_packages() lista con filtros (category, type, is_active)"
    - "Método update_package() actualiza campos opcionales"
    - "Método deactivate_package() hace soft delete (is_active=False)"
    - "Servicios NO hacen commit - handlers gestionan transacciones"
    - "ContentService integrado en ServiceContainer con lazy loading"
  artifacts:
...
@bot/database/models.py (ContentPackage model created in 05-02)
...
print('✅ ContentService structure verified')
"
  </verify>
  <done>
...
1. Add _content_service private attribute in __init__:
   After line 55 (after self._role_detection_service = None):
   Add: self._content_service = None

2. Add content property after the role_detection property:
```python
    # ===== CONTENT SERVICE =====
...
print('✅ ServiceContainer.content property verified')
"
  </verify>
  <done>
...
print('✅ ContentService export verified')
"
  </verify>
  <done>
...
async def test_content_service():
    async with get_session() as session:
        async with session.begin():
            ...
            print(f'✅ Package created: {pkg.id} - {pkg.name}')
            ...
            print(f'✅ Listed {len(packages)} active packages')
            ...
            print('✅ Package deactivated successfully')

    print('✅ All ContentService CRUD operations work correctly')
...
async def test_container():
    async with get_session() as session:
        ...
        print('✅ ContentService loaded from container')
        ...
        print('✅ ContentService cached correctly')
