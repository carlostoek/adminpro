...
must_haves:
  truths:
    - "Base de datos tiene tabla ContentPackage con todos los campos requeridos"
    - "Base de datos tiene tabla UserInterest con relación a ContentPackage"
    - "Base de datos tiene tabla UserRoleChangeLog para auditoría de cambios de rol"
    - "ContentPackage usa Numeric(10, 2) para precio (no Float)"
    - "ContentPackage tiene índices compuestos en (category, is_active) y (type, is_active)"
    - "UserInterest tiene constraint único (user_id, package_id) para deduplicación"
    - "UserRoleChangeLog registra previous_role, new_role, changed_by, reason, change_source"
  artifacts:
