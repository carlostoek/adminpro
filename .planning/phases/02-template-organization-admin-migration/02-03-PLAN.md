...
must_haves:
  truths:
    ...
    - "Main menu shows 2 variations for greeting with random selection"
...
1. `admin_menu_greeting(self, is_configured: bool, missing_items: List[str] = None) -> Tuple[str, InlineKeyboardMarkup]`
   - 2 greeting variations (equal random):
     * "Ah, el custodio de los dominios de Diana..."
     * "Bienvenido de nuevo al sanctum..."
   - Conditional body based on is_configured:
     * If configured: "Todo está en orden. ¿Qué aspecto del reino requiere su atención hoy?"
...
2. `config_menu(self) -> Tuple[str, InlineKeyboardMarkup]`
   ...
   - "Desde aquí puede ajustar los parámetros del reino..."
...
**Voice guidelines:**
- Use "custodio" or "guardián" for admin
- Use "reino" for system/domain
- Use "calibración" for configuration
...
- These will be updated in Task 3 with Lucien voice
...
- "📊 Dashboard Completo" → keep (already good)
- "📺 Gestión Canal VIP" → "👑 Círculo Exclusivo VIP"
- "📺 Gestión Canal Free" → "📺 Vestíbulo de Acceso"
- "⚙️ Configuración" → "⚙️ Calibración del Reino"
- "💰 Tarifas" → "💰 Planes de Suscripción"
- "📊 Estadísticas" → "📈 Observaciones del Reino"
...
Change button text:
- "📊 Ver Estado de Configuración" → "📊 Estado del Reino"
- "⚙️ Configurar Reacciones VIP" → "👑 Reacciones del Círculo"
- "⚙️ Configurar Reacciones Free" → "📺 Reacciones del Vestíbulo"
- "🔙 Volver al Menú Principal" → keep
...
- "📊 Ver Stats VIP Detalladas" → "📊 Observaciones del Círculo"
- "📊 Ver Stats Free Detalladas" → "📊 Observaciones del Vestíbulo"
- "🎟️ Ver Stats de Tokens" → "🎟️ Registro de Invitaciones"
- "🔄 Actualizar Estadísticas" → "🔄 Actualizar Observaciones"
</action>
...
5. Voice consistency: "custodio", "reino", "calibración" used
6. 2 greeting variations implemented in admin_menu_greeting()
7. Keyboards in utils/keyboards.py updated with Lucien voice
8. admin_main_menu_keyboard uses "Círculo Exclusivo VIP" and "Vestíbulo de Acceso"
...
2. Main menu shows 2 variations for greeting
