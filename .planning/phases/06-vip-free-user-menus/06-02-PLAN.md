...
must_haves:
  truths:
    ...
    - "Each premium package has 'Me interesa' button that creates UserInterest record"
...
2. Add callback handler for VIP premium section:
   ...
   async def handle_vip_premium(callback: CallbackQuery, **kwargs):
       ...
       try:
           ...
           # Generate premium section message
...
3. Add callback handler for "Me interesa" button:
   ...
   async def handle_package_interest(callback: CallbackQuery, **kwargs):
       ...
       try:
           ...
           logger.info(f"❤️ Usuario {user.id} interesado en paquete {package_id}")
...
3. `handle_vip_premium` gets VIP_PREMIUM packages from ContentService
4. `handle_package_interest` parses package ID from callback data
...
3. Test VIP menu integration:
   ...
   print("✅ VIP handlers import successful")
   print(f"✅ vip_callbacks_router has {len(vip_callbacks_router.handlers)} handlers")

   # Test callback patterns are registered
   from aiogram import Dispatcher
   dp = Dispatcher()
   dp.include_router(vip_callbacks_router)
   print("✅ vip_callbacks_router can be included in dispatcher")

   print("\n🎉 VIP handlers integration test passed")
   ```
...
<success_criteria>
1. ✅ VIP menu handler enhanced to use UserMenuProvider with subscription info
2. ✅ VIP callback handlers created for premium section and interest registration
3. ✅ Navigation callbacks implemented ("Volver", "Salir")
4. ✅ ContentService integration for getting VIP_PREMIUM packages
5. ✅ VIP handlers package exports callback router
