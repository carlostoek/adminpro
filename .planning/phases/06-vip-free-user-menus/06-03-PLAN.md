...
must_haves:
  truths:
    ...
    - "Each free package has 'Me interesa' button that creates UserInterest record"
...
3. Enhanced implementation:
   ...
   async def show_free_menu(message: Message, data: Dict[str, Any]):
       ...
       logger.info(f"🆓 Menú Free mostrado a {user.id} (voz de Lucien)")
   ```
...
1. Create the file with imports and router setup:
   ...
   Responsabilidades:
   - Manejar callback "free:content" - mostrar sección "Mi Contenido"
   - Manejar callback "free:vip_info" - mostrar información del canal VIP
...
2. Add callback handler for Free content section ("Mi Contenido"):
   ...
   async def handle_free_content(callback: CallbackQuery, **kwargs):
       ...
       try:
           ...
           # Generate content section message
...
3. Add callback handler for VIP channel info:
   ...
   async def handle_vip_info(callback: CallbackQuery, **kwargs):
       ...
       try:
           ...
           if vip_channel_id:
               message_text += (
                   f"El sanctum está disponible para aquellos que posean "
                   f"el token de acceso.\n\n"
                   f"<b>Beneficios:</b>\n"
                   f"• Contenido exclusivo y anticipado\n"
                   f"• Comunidad privada de miembros\n"
                   f"• Acceso a Diana para consultas directas\n"
                   f"• Contenido premium adicional\n\n"
                   f"<i>Para unirse, necesitará un token de invitación.</i>"
               )
           else:
               message_text += (
                   f"El sanctum aún no ha sido configurado por los custodios.\n\n"
           ...
           keyboard.button(text="⬅️ Volver", callback_data="menu:back")
...
4. Add callback handler for social media/free content:
   ...
   async def handle_social_media(callback: CallbackQuery):
       ...
       try:
           ...
           logger.info(f"🆓 Redes sociales mostradas a {callback.from_user.id}")
...
5. Reuse interest handler from VIP callbacks (same pattern):
   ...
   async def handle_package_interest(callback: CallbackQuery, **kwargs):
       ...
       Reutiliza lógica de VIP callbacks.
       """
       data = kwargs.get("data", {})
       container = data.get("container")
       user = callback.from_user
       ...
       try:
           ...
           # TODO: Create UserInterest record (Phase 8)
           logger.info(f"❤️ Usuario Free {user.id} interesado en paquete {package_id}")
...
6. Add navigation handlers (reuse from VIP pattern):
   ...
   async def handle_menu_back(callback: CallbackQuery, **kwargs):
       ...
       try:
           ...
           logger.error(f"Error volviendo al menú Free para {user.id}: {e}")
   ...
   async def handle_menu_exit(callback: CallbackQuery):
       ...
       try:
           ...
           logger.error(f"Error cerrando menú Free para {callback.from_user.id}: {e}")
...
3. `handle_free_content` gets FREE_CONTENT packages from ContentService
4. `handle_vip_info` shows VIP channel subscription information
5. `handle_social_media` displays social media and free content
6. Interest handler reuses VIP pattern for consistency
...
2. Test Free menu integration:
   ...
   print("✅ Free handlers import successful")
   print(f"✅ free_callbacks_router has {len(free_callbacks_router.handlers)} handlers")

   # Test callback patterns
   from aiogram import Dispatcher
   dp = Dispatcher()
   dp.include_router(free_callbacks_router)
   print("✅ free_callbacks_router can be included in dispatcher")
   ...
   for req in required_handlers:
       if req in handler_names:
           print(f"✅ {req} handler registered")
       else:
           print(f"❌ {req} handler missing")

   print("\n🎉 Free handlers integration test passed")
   ```
...
<success_criteria>
1. ✅ Free menu handler enhanced to use UserMenuProvider
2. ✅ Free callback handlers created for content browsing, VIP info, and social media
3. ✅ "Mi Contenido" section gets FREE_CONTENT packages from ContentService
4. ✅ VIP info section shows subscription benefits and channel status
5. ✅ Social media section displays Diana's social links
6. ✅ Interest handler reuses VIP pattern for consistency
