...
2. Add waiting_for_edit state to ContentPackageStates (in bot/states/admin.py):
```python
waiting_for_edit = State()  # For inline prompt editing
```

3. Process edit (message):
```python
...
2. waiting_for_edit state added to ContentPackageStates
3. Process edit handler: process_content_edit
