...
@pytest.mark.asyncio
async def test_vip_flow_complete(mock_bot, test_session, test_user):
    """
    ...
    # Commit para que el token sea visible en la BD
    ...
    print(f"     Debug: success={success}, msg={msg}, subscriber={subscriber}")
    assert success == True, f"Expected success=True, got success={success}, msg={msg}"
    assert subscriber is not None
    assert subscriber.user_id == user_id
    assert subscriber.status == "active"
    print(f"     OK: Token canjeado")

    # Commit para que el suscriptor sea visible en la BD
    await test_session.commit()
...
@pytest.mark.asyncio
async def test_free_flow_complete(mock_bot, test_session, test_free_user):
    """
...
@pytest.mark.asyncio
async def test_vip_expiration(mock_bot, test_session, test_vip_user):
    """
...
@pytest.mark.asyncio
async def test_token_validation_edge_cases(mock_bot, test_session):
    """
...
@pytest.mark.asyncio
async def test_duplicate_free_request_prevention(mock_bot, test_session, test_user):
    """
